...
show_usage()
{
    printf( "\n"
            ...
            "\n"
            "  About iClient:\n\n"
            "\tThis program is a C implementation to DCSM Client Authentication.\n"
...
//    printf("PackType: %d\n", pType);
    switch(pType){
        ...
        case EAP_FAILURE:
            ...
            print_server_info (packet, packetinfo->caplen);
            if(auth_times>AUTH_TIMES || auth_times==0)
...
init_frames()
{
   ...
   /*print_hex (local_info_tailer, tail_len);*/
    ...
    eap_response_ident = malloc (14 + 9 + username_length + tail_len);
   ...
   /*print_hex (eap_response_ident, 14 + 9 + username_length + tail_len);*/
    ...
    uint8_t eap_resp_md5_head[10] = {0x01, 0x00,
                                   ...
                                   0x04, 0x10};
    eap_response_md5ch = malloc (14 + 4 + 6 + 16 + username_length + tail_len);
    ...
    uint8_t eap_resp_md5keep_head[9] = {0x01, 0x00,
                                   ...
                                   0xfa};
    eap_response_md5keep = malloc (14 + 4 + 5 + 16 + tail_len);
...
void init_arguments(int *argc, char ***argv)
{
    ...
    while (1) {
        ...
        int option_index = 0;
        c = getopt_long ((*argc), (*argv), "u:p:g:d:t:hbl",
        ...
        switch (c) {
            case 0:
               break;
            case 't':
                AUTH_TIMES = strtol(optarg, NULL, 10);
                break;
            case 'b':
                background = 1;
                break;
